...
func (c *Controller) fetchSpeciesSummaryData(ctx echo.Context, startDate, endDate string, sourceIDs []uint) ([]datastore.SpeciesSummaryData, time.Duration, error) {
	...
	queryCtx, cancel := withAnalyticsTimeout(ctx)
	defer cancel()
	summaryData, err := c.DS.GetSpeciesSummaryData(queryCtx, startDate, endDate, sourceIDs...)
...
func (c *Controller) GetNewSpeciesDetections(ctx echo.Context) error {
	...
	queryCtx, cancel := withAnalyticsTimeout(ctx)
	defer cancel()
	newSpeciesData, err := c.DS.GetNewSpeciesDetections(queryCtx, startDate, endDate, limit, offset, sourceIDs...)
	if err != nil {
